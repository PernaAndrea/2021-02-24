	...
	public List<Adiacenza> getAdiacenze(Match m, Map<Integer, Player> idMap) {
		...
		try {
			...
			while (res.next()) {
				...
				if(idMap.containsKey(res.getInt("a2.PlayerID")) && idMap.containsKey(res.getInt("a1.PlayerID"))) {
					Adiacenza a = new Adiacenza(idMap.get(res.getInt("a1.PlayerID")),idMap.get(res.getInt("a2.PlayerID")),res.getDouble("peso"));
					result.add(a);
				}
